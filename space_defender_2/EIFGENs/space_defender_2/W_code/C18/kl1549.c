...
#define uc1 uc1x.it_c1
	...
	RTEAA(l_feature_name, 1548, Current, 4, 0, 21694);
	RTSA(dtype);
	RTSC;
	RTME(dtype, 0);
	RTGC;
	RTDBGEAA(1548, Current, 21694);
	RTIV(Current, RTAL);
...
#define uc1 uc1x.it_c1
	...
	RTEAA(l_feature_name, 1548, Current, 0, 0, 21695);
	RTSA(dtype);
	RTSC;
	RTME(dtype, 0);
	RTGC;
	RTDBGEAA(1548, Current, 21695);
	RTIV(Current, RTAL);
	if ((RTAL & CK_REQUIRE) || RTAC) {
		RTHOOK(1);
		RTCT("is_open_read", EX_PRE);
...
{
	GTCX
	RTOTC (F1549_15642,21697,RTMS_EX_H("\012",1,10));
}
